...
from services.submission import compute_submission_score
...
async def cast_vote(
    ...
    author_result = await session.get(Character, submission.character_id)
    ...
    vote = await cast_or_update_vote(voter, submission, data.stars, session)
...
async def get_vote_summary(
    ...
    count_result = await session.execute(
        select(func.count()).select_from(Vote).where(Vote.submission_id == praxis_id)
    )
    total_votes = count_result.scalar_one()

    avg_result = await session.execute(
        select(func.avg(Vote.stars)).where(Vote.submission_id == praxis_id)
    )
    avg_stars = float(avg_result.scalar_one_or_none() or 0.0)
    total_score = await compute_submission_score(submission, session)
...
async def list_voters(
    ...
    result = await session.execute(
        ...
        .where(Vote.submission_id == praxis_id)
