...
**Notes:**
- Letters (welcome letters + invitation letters) are *not* a level-table unlock. Welcome letters fire when a level 2+ UA character completes a task associated with a faction. Formal invitation letters are triggered when all three conditions are met: (1) 2 tasks completed for that faction, (2) 50 points earned from that faction's tasks, and (3) a "Pledge allegiance to [faction]" praxis submitted with at least 2 votes and no flags.
