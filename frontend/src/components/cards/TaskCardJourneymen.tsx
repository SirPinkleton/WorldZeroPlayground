...
export default function TaskCardJourneymen({ task, onSignup }: Props) {
  return (
    <div style={{ paddingTop: 26, position: 'relative', minWidth: 108, maxWidth: 132, flex: '0 1 118px' }}>
      ...
      >
        {/* Hazard stripe */}
        <div style={{ height: 3, backgroundImage: `repeating-linear-gradient(90deg, var(--faction-journeymen-stripe-red) 0, var(--faction-journeymen-stripe-red) 8px, ${factionCssVar('journeymen', 'card-bg')} 8px, ${factionCssVar('journeymen', 'card-bg')} 16px, var(--faction-journeymen-stripe-amber) 16px, var(--faction-journeymen-stripe-amber) 24px, ${factionCssVar('journeymen', 'card-bg')} 24px, ${factionCssVar('journeymen', 'card-bg')} 32px)` }} />
        ...
        <div style={{ padding: '8px 10px 10px' }}>
          ...
          {task.description && (
            <div className="card-description" style={{ color: factionCssVar('journeymen', 'card-muted') }}>
              {task.description}
            </div>
          )}
